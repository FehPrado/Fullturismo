...
if(isset($_POST['btnEnviar'])){
	...
	if(base64_decode ($_GET['ref'])=='NOVO'){
		...
		(nome,cpf,email,sexo,nascimento,FKprofissao,endereco,bairro,senha,cidade,estado) 
		VALUES (
	...
	mysqli_query($conecta, 
		...
		FKprofissao = '" . trim($_POST['txtProfissao']) . "' , 
...
if(!empty($_GET["PKID"])){
	$select = mysqli_query($conecta, "SELECT * FROM funcionario WHERE PKID=". base64_decode($_GET["PKID"]));
	 	...
	 	<div class="page-content">
			...
			<div class="breadcrumb-line">
				<ul class="breadcrumb">
					...
					<li><a href="index.php">Funcionáios</a></li>
						...
						<div class="form-group">
							<div class="row">
								...
								<div class="col-md-4">
                                    ...
                                    <select name="txtProfissao" data-placeholder="Selecione..." class="select-full" tabindex="2">
										<option value=""></option>
                                                                                
                                                               
                                                                                
										<?php 
											$selectProfissao = mysqli_query($conecta, "SELECT * FROM profissoes ORDER BY nome");
										   
										    if(mysqli_num_rows($selectProfissao)>=1){
											while($resultProfissao = mysqli_fetch_assoc($selectProfissao)){
											?>
										<option value="<?php echo $resultProfissao["PKprofissao"]?>"
												<?php if($resultProfissao["PKprofissao"]==$result["FKprofissao"]){ echo "selected";} ?> >
											
											<?php echo $resultProfissao["nome"] ?></option>
											<?php
							...
							<div class="row">
								...
								<div class="col-md-4">
                                    ...
                                    <input value="<?php echo $result["bairro"]?>" name="txtBairro" type="text" class="styled form-control" placeholder="Jardim das Nações">  
                                                                   ...
                                                                   <div class="col-md-4">
                                                                        <label>Senha:</label>
	                                <input value="<?php echo $result["senha"]?>" type="text" name="txtsenha" class="form-control" ">
								</div> 
							</div>
						</div>
