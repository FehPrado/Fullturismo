...
if(isset($_POST['btnEnviar'])){
	...
	if(base64_decode($_GET['ref'])=='NOVO') {
		...
		"INSERT INTO estado 
		(nome,descricao, foto, avaliacao, lancamento, pais)	
		VALUES (
...
if(!empty($_GET["PKID"])) {
	$select = mysqli_query($conecta, "SELECT * FROM estado WHERE PKID = " . base64_decode($_GET["PKID"]));
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-2">
                                ...
                                <select name="txtpais" class="form-control">
                                    <option></option>
                                    <?php
                                        $buscaPais=mysqli_query($conecta, "SELECT * FROM pais ORDER BY nome");
                                        while($resultPais=mysqli_fetch_assoc($buscaPais)){
                                    ?>
                                    <option value="<?php echo $resultPais["PKID"]?>"><?php echo $resultPais["nome"]?></option>
                                    <?php } ?>
                                </select>
                                </textarea>
                            </div>
